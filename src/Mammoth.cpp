...
Mammoth::Mammoth(const std::vector<glm::vec2> waypoints, Level* level){
    ...
    for(int i=0;i<=5;i++){
        move_down.push_back(RESOURCE_DIR"/output_images/Mammoth/tile_0_" + std::to_string(i) +".png");
    }
    for(int i=0;i<=5;i++){
        move_left.push_back(RESOURCE_DIR"/output_images/Mammoth/tile_1_" + std::to_string(i) +".png");
    }
    for(int i=0;i<=5;i++){
        move_right.push_back(RESOURCE_DIR"/output_images/Mammoth/tile_2_" + std::to_string(i) +".png");
    }
    for(int i=0;i<=5;i++){
