...
在無盡關卡中，隨著守衛者等級提升，其攻擊速度也會隨之加快。然而，原本設計中，`Button`（子彈）僅作為附加於 `Guard`（守衛者）的一個物件，會根據角色的攻擊動作產生並朝向敵人發射。  
但由於攻擊間隔縮短，導致子彈尚未碰到敵人，`Guard` 就已重新發動下一次攻擊，進而重置了子彈。這使得子彈無法順利命中敵人，也無法正確扣除敵人血量。

#### 2. **Enemy類別問題**

隨著遊戲的開發，有越來越多的功能出現，當初的Enemy類別已經不能滿足我們的需求。
所以重新做了一個Enemy，讓他可以處理血量、金錢等，讓level類別變得更乾淨，同時也讓不同的Enemy可以有更多的變化。

#### 解決方式

改以 `vector` 容器儲存每次產生的 `Button`，使每個子彈成為獨立個體，分別執行其 `Update` 和 `Draw` 流程。  
如此一來，即使守衛者在短時間內再次攻擊，前一顆子彈仍能持續飛行並有效命中目標，不會被覆蓋或重置，成功解決了命中失效的問題。
### 自評
